...
int ls_Insertar(TListaSimple *pLs, TMovimiento_Ls M, void* pE){
  ...
  if ((pLs->Primero == NULL) || (M==LS_PRIMERO) || ((M==LS_ANTERIOR) && (pLs->Primero==pLs->Corriente))) { /* Si est vaca o hay que insertar en el primero o hay que insertar en el anterior y el actual es el primero */
