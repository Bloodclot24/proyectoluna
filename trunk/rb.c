...
rb_probe (struct rb_table *tree, void *item)
{
  ...
  n->rb_data = item;
  n->repeticiones=0;    /* Para saber cuantas veces se intento insertar un valor */
...
/* Deletes from |tree| and returns an item matching |item|.
   Returns a null pointer if no matching item found. */
void *
rb_remover_mayor_igual (struct rb_table *tree, const void *item)
{
  struct rb_node *p;
  struct rb_node *anterior=NULL;
  ...
  if(anterior == NULL)
    return NULL;
  if(anterior->repeticiones > 0){
    anterior->repeticiones--;
    return anterior->rb_data;
  }
  else return rb_delete(tree, anterior->rb_data);
